...
def bom_creation_tool(folder_path):
    ...
    main_subassy_all_col_df =  df_rozpiska.iloc[[0]]
    
    if (df_rozpiska.iloc[0]['BDE_NR'] == '0') or pd.isna(df_rozpiska.iloc[0]['BDE_NR']):
        df_rozpiska.at[0, 'BDE_NR'] == '0'
    ...
    for index, row in main_subassy_all_col_df.iterrows():
        ...
        # result_list = pd.DataFrame(result_df)
        # csv_path = os.path.join(folder_path, f"{ms_zeich}.csv")
